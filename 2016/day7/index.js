...
const strings = `dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
const part1 = () => {
  const hasAbba = (str) => {
    ...
    for (let i = 0; i < chars.length - 2; i++) {
      const currentPair = [chars[i], chars[i + 1]];
      const nextPair = [chars[i + 2], chars[i + 3]].reverse();
      if (
        currentPair[0] !== currentPair[1] &&
        currentPair.join("") === nextPair.join("")
      ) {
        abba = currentPair.join("") + nextPair.reverse().join("");
        return true;
      }
    }
    return false;
...
const part2 = () => {
  const getAbas = (str) => {
    ...
    str.split("").forEach((c, i) => {
      if (str[i + 2] === c && c !== str[i + 1]) {
        abas.push([c, str[i + 1], c].join(""));
      }
    });
    return abas;
  };
  const invertAba = (aba) => {
    return [aba[1], aba[0], aba[1]].join("");
  };
  const testStrings = (str) => {
    const parts = str.split(/\[[a-z]+\]/);
    let valid = false;
    const inBrackets = str
      .match(/\[([a-z]+)\]/g)
    ...
    const abas = parts.map(getAbas);
    inBrackets.forEach((p) => {
      abas
        .filter((x) => x.length > 0)
        .flat()
        .forEach((aba) => {
          if (p.includes(invertAba(aba))) {
            valid = true;
          }
        });
    });
