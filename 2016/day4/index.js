...
const allRooms = `gbc-frperg-pubpbyngr-znantrzrag-377[rgbnp]
...
const isValid = (r) => {
  const checkSum = r.match(/\[([a-z]+)\]/)[1];
  const [part1, checkSumPart] = r.split("[");
  const sectorIdMatch = part1.match(/\d+/)[0];
  const [rest] = part1.split(sectorIdMatch);
  const chars = rest.split("-").filter(Boolean).join("").split("");
  // console.log({ checkSum, sectorIdMatch, rest, chars });

  const score = chars.reduce((acc, cur) => {
    if (!acc[cur]) {
      acc[cur] = {
        score: 1,
      };
      ...
      return {
        ...acc,
        [cur]: {
          score: acc[cur].score + 1,
        },
      };
    }
  }, {});

  const orderByScore = Object.keys(score).sort((a, b) => {
  ...
  const groups = {};
  orderByScore.forEach((g) => {
    if (!groups[score[g]?.score]) {
      groups[score[g].score] = [g];
    } else {
      groups[score[g].score].push(g);
    }
  });
  const groupListOrdered = Object.keys(groups)
    .map(Number)
    .sort((a, b) => b - a);

  const ordered = groupListOrdered
    .map((x) => {
      if (groups[x].length > 1) {
        return groups[x].sort();
      } else {
        return groups[x];
      }
    })
    ...
    .join("");
  // console.log(ordered, );
  return ordered.includes(checkSum);
};

let score = 0;
allRooms.split("\n").forEach((r) => {
  if (isValid(r)) {
    const sectId = Number(r.match(/\d+/)[0]);
    // console.log(r, sectId);
    score += sectId;
  }
});
console.log(score);
...
const rotateRoom = (l) => {
  const [textPart] = l.split(/\d+/);
  const words = textPart.split("-").filter(Boolean);
  const rotateWith = Number(l.match(/\d+/)[0]);

  const rotatedDone = [];

  words.forEach((word) => {
    const rotWord = word.split("").map((c) => {
      const newC = alpha[(alpha.indexOf(c) + rotateWith) % alpha.length];
      return newC;
    });
    rotatedDone.push(rotWord.join(""));
  });
  const decrypted = rotatedDone.join(" ");
  if (decrypted.includes("storage")) {
    if (decrypted === "northpole object storage") {
      console.log(decrypted, rotateWith);
    }
  }
};

rotateRoom("qzmt-zixmtkozy-ivhz-343[apa]");
const validRooms = allRooms.split("\n").filter(isValid);
validRooms.map(rotateRoom);
